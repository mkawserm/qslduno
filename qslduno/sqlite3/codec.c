...
void CodecGetHASHBinary(Codec *codec, unsigned char *data, int length, unsigned char *digest)
{
    if(GLOBAL_CODEC_TYPE==1){
        CodecGetMD5Binary(codec,data,length,digest);
    }
    else if(GLOBAL_CODEC_TYPE==2){
        CodecGetSHABinary(codec, data, length, digest);
    }
}
...
void
CodecAES(Codec* codec, int page, int encrypt, unsigned char encryptionKey[MAX_KEYLENGTH],
         unsigned char* datain, int datalen, unsigned char* dataout)
{
  ...
  unsigned char pagekey[MAX_KEYLENGTH];
  unsigned char nkey[MAX_KEYLENGTH+4+4];
  ...
  if(GLOBAL_CODEC_TYPE==1){//aes-128
      keyLength = 16;
  }
  else{//aes-256
      keyLength = 32;
  }
  nkeylen = keyLength + 4 + 4;
  direction = (encrypt) ? RIJNDAEL_Direction_Encrypt : RIJNDAEL_Direction_Decrypt;
  len = 0;
...
//#endif
  CodecGetHASHBinary(codec, nkey, nkeylen, pagekey);
...
//#endif
  ...
  if(GLOBAL_CODEC_TYPE == 1){//AES_128
      RijndaelInit(codec->m_aes, RIJNDAEL_Direction_Mode_CBC, direction, pagekey, RIJNDAEL_Direction_KeyLength_Key16Bytes, initial);
  }
  else{//AES_256
      RijndaelInit(codec->m_aes, RIJNDAEL_Direction_Mode_CBC, direction, pagekey, RIJNDAEL_Direction_KeyLength_Key32Bytes, initial);
...
CodecGenerateEncryptionKey(Codec* codec, char* userPassword, int passwordLength, 
                           unsigned char encryptionKey[MAX_KEYLENGTH])
{
    if(GLOBAL_CODEC_TYPE==1){//AES_128
